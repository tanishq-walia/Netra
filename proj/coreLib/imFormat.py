...
def rgb2hsv(rgbImage):
	return cv2.cvtColor(rgb2bgr(rgbImage), cv2.COLOR_BGR2HSV)
...
def equalizeHistogram(grayscaleImage):
	# create a CLAHE object (Arguments are optional).
	clahe = cv2.createCLAHE(clipLimit=2.0, tileGridSize=(8,8))
	cl1 = clahe.apply(grayscaleImage)
	return cl1

def equalize(bgr_image):
	l, a, b = cv2.split(cv2.cvtColor(bgr_image, cv2.COLOR_BGR2LAB))
	lmod = equalizeHistogram(l)
	output = cv2.cvtColor(cv2.merge([lmod,a,b]), cv2.COLOR_LAB2BGR)
	return output
